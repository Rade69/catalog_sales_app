...
def get_pixmap(icon_name: str, color: str = "#9ca3af", size: int = 20) -> QPixmap:
    """
    Renderuje SVG ikonicu u QPixmap sa određenom bojom.

    Args:
        icon_name: Naziv ikonice iz ICONS dict-a
        color: HEX boja za stroke
        size: Veličina u pikselima
    ...
    svg = ICONS.get(icon_name, ICONS["dashboard"])
    svg_colored = svg.replace('stroke="currentColor"', f'stroke="{color}"')
    svg_bytes = svg_colored.strip().encode("utf-8")
...
def create_icon_label(icon_name: str, color: str = "#9ca3af", size: int = 18) -> QLabel:
    """
    Kreira QLabel sa SVG ikonicom za korištenje na dugmad.

    Args:
        icon_name: Naziv ikonice iz ICONS dict-a
        color: HEX boja za stroke
        size: Veličina ikonice u px

    Returns:
        QLabel sa renderovanom ikonicom
    """
    from PySide6.QtWidgets import QLabel
    
    label = QLabel()
    label.setFixedSize(size, size)
    pixmap = get_pixmap(icon_name, color, size)
